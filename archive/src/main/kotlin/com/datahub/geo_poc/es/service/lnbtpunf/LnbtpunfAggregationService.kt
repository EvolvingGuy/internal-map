...
/**
 * LNBTPUNF (Land Nested Building Trade Point Uniform No Forcemerge) Aggregation 서비스
 * 4개 파티션 인덱스 동시 쿼리 (lnbtpunf_*)
 */
@Service
class LnbtpunfAggregationService(
    ...
    companion object {
        const val INDEX_PATTERN = "lnbtpunf_*"
    ...
    private fun aggregate(field: String, size: Int, bbox: BBoxRequest, filter: LcAggFilter): LcAggResponse {
        ...
        log.info("[LNBTPUNF Agg] field={}, regions={}, totalCount={}, hasFilter={}, index={}, ES took={}ms, total={}ms",
    ...
    private fun logProfile(response: org.opensearch.client.opensearch.core.SearchResponse<Void>, field: String) {
        ...
        log.info("[LNBTPUNF Profile] field={}, shards={}", field, shards.size)
        ...
        shards.forEach { shardProfile ->
            ...
            shardProfile.searches().forEach { search ->
                search.query().forEach { queryProfile ->
                    ...
                    log.info("[LNBTPUNF Profile] {} - {} - {}ms", shardId, queryProfile.type(), timeMs)
                }
            }
            shardProfile.aggregations().forEach { aggProfile ->
                val timeMs = String.format("%.2f", aggProfile.timeInNanos() / 1_000_000.0)
                log.info("[LNBTPUNF Profile] {} - agg:{} - {}ms", shardId, aggProfile.type(), timeMs)
            }
        }
    }
}
