# LNBP (Land Nested Building Point)
필지 단위 인덱스 - LNB의 경량 버전, geo_shape 대신 geo_point만 사용하여 용량 최소화

## 인덱스 정보
| key        | value                        |
|------------|------------------------------|
| IndexName  | `land_nested_building_point` |
| Shards     | 3                            |
| Replicas   | 0                            |
...
| Method | Endpoint                   |
|--------|----------------------------|
| PUT    | `/api/es/lnbp/reindex`     | 전체 재인덱싱 (백그라운드)
| PUT    | `/api/es/lnbp/forcemerge`  | 세그먼트 병합
| GET    | `/api/es/lnbp/count`       | 인덱스 문서 수
| DELETE | `/api/es/lnbp`             | 인덱스 삭제

### 집계 API
| Method | Endpoint                 |
|--------|--------------------------|
| GET    | `/api/es/lnbp/agg/sd`    | 시도별 집계
| GET    | `/api/es/lnbp/agg/sgg`   | 시군구별 집계
| GET    | `/api/es/lnbp/agg/emd`   | 읍면동별 집계

## 인덱싱 방식
LNB와 동일한 EMD 단위 병렬 처리 방식 사용.
...
[1] ensureIndexExists()
    ├─ 기존 인덱스 존재 시 삭제
    └─ 신규 인덱스 생성 (shard:3, replica:0, buildings nested, geometry 없음)
           ↓
[2] EMD 분배
...
## 관련 파일
- Controller: `controller/rest/LnbpRestController.kt`, `controller/rest/LnbpAggRestController.kt`
- Web: `controller/web/LnbpAggWebController.kt`
- Indexing: `es/service/LnbpIndexingService.kt`
- Aggregation: `es/service/LnbpAggregationService.kt`
- Document: `es/document/land/LnbpDocument.kt`
- Template: `templates/es/lnbp/agg.ftl`
- Docker: `docker-compose.lnbp.yml` (volume: `geo_poc_es_lnbp`)
