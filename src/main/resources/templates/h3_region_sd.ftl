...
<body>
    ...
    <script>
        ...
        function fetchData() {
            ...
            fetch(`/api/h3/region/sd?${r"${params}"}`)
                .then(res => res.json())
                .then(data => {
        ...
        function buildLabelHtml(count, name) {
            return '<div style="position:relative;"><div style="background:#dc2626;color:#fff;padding:6px 12px;border-radius:12px;font-size:13px;font-weight:bold;white-space:nowrap;transform:translate(-50%,-50%);text-align:center;">' +
