...
<body>
    ...
    <script>
        ...
        function fetchH3Data() {
            ...
            fetch(`/api/h3/cells/sd?${r"${params}"}`)
                .then(res => res.json())
                .then(data => {
        ...
        function buildLabelHtml(count) {
            return '<div style="position:relative;"><div style="background:#dc2626;color:#fff;padding:4px 10px;border-radius:12px;font-size:12px;font-weight:bold;white-space:nowrap;transform:translate(-50%,-50%);">' +
