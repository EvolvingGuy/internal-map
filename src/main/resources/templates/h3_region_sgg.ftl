...
<body>
    ...
    <script>
        ...
        function fetchData() {
            ...
            fetch(`/api/h3/region/sgg?${r"${params}"}`)
                .then(res => res.json())
                .then(data => {
        ...
        function buildLabelHtml(count, name) {
            return '<div style="position:relative;"><div style="background:#16a34a;color:#fff;padding:5px 10px;border-radius:10px;font-size:12px;font-weight:bold;white-space:nowrap;transform:translate(-50%,-50%);text-align:center;">' +
