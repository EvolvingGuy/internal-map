...
<body>
    ...
    <script>
        ...
        function fetchData() {
            ...
            fetch(`/api/region-count/sgg?${r"${params}"}`)
                .then(res => res.json())
                .then(data => {
        ...
        function buildLabelHtml(count, regionName) {
            return '<div style="position:relative;"><div style="background:#7c3aed;color:#fff;padding:6px 12px;border-radius:8px;font-size:13px;font-weight:bold;white-space:nowrap;transform:translate(-50%,-50%);text-align:center;">' +
