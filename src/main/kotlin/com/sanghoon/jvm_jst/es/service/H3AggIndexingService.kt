...
import com.sanghoon.jvm_jst.es.document.H3AggDocument
...
class H3AggIndexingService(
    ...
    fun reindex(): Map<String, Any> {
        ...
        log.info("[H3Agg] EMD - PostgreSQL 조회: {}건, {}ms", emdEntities.size, emdDbTime)

        val emdIndexStart = System.currentTimeMillis()
        val emdDocs = emdEntities.map { H3AggDocument.fromEmd10(it) }
        bulkIndexChunked(emdDocs, 10000)
        val emdIndexTime = System.currentTimeMillis() - emdIndexStart
        log.info("[H3Agg] EMD - ES 인덱싱: {}건, {}ms", emdDocs.size, emdIndexTime)
        ...
        log.info("[H3Agg] SGG - ES에서 집계: {}건, {}ms", sggDocs.size, sggTime)
        bulkIndex(sggDocs)
        ...
        log.info("[H3Agg] SD - ES에서 집계: {}건, {}ms", sdDocs.size, sdTime)
        bulkIndex(sdDocs)
     ...
     */
    private fun aggregateFromEs(sourceLevel: String, targetRes: Int, targetLevel: String): List<H3AggDocument> {
        val docs = mutableListOf<H3AggDocument>()
        ...
        while (true) {
            ...
            for (hit in hits) {
                ...
                val key = "${parentH3}_${doc.regionCode}"

                aggMap.compute(key) { _, existing ->
                    if (existing == null) {
                        AggData(parentH3, doc.regionCode, doc.cnt.toLong(), doc.sumLat, doc.sumLng)
                    } else {
                        existing.copy(
            ...
            log.info("[H3Agg] {} → {} 집계 중: {}건 처리, 현재 집계 키: {}개",
                sourceLevel, targetLevel, hits.size, aggMap.size)
        }

        // 집계 결과를 문서로 변환
        for ((_, agg) in aggMap) {
            docs.add(H3AggDocument(
    ...
    private fun bulkIndex(docs: List<H3AggDocument>) {
        ...
        if (response.errors()) {
            ...
            log.warn("[H3Agg bulkIndex] 일부 실패: {}/{}", failedCount, docs.size)
        }
    }

    private fun bulkIndexChunked(docs: List<H3AggDocument>, chunkSize: Int) {
        docs.chunked(chunkSize).forEachIndexed { idx, chunk ->
            bulkIndex(chunk)
            if ((idx + 1) % 10 == 0) {
                log.info("[H3Agg] EMD 청크 진행: {}/{}", (idx + 1) * chunkSize, docs.size)
            }
        }
    }
    ...
    private fun ensureIndexExists() {
        ...
        if (exists) {
            ...
            log.info("[H3Agg] 기존 인덱스 삭제")
        }

        log.info("[H3Agg] 인덱스 생성 - {}", INDEX_NAME)
    ...
    fun reindexEmd(): Map<String, Any> {
        ...
        log.info("[H3Agg] EMD - PostgreSQL 조회: {}건, {}ms", emdEntities.size, dbTime)

        val indexStart = System.currentTimeMillis()
        val emdDocs = emdEntities.map { H3AggDocument.fromEmd10(it) }
        bulkIndexChunked(emdDocs, 10000)
        val indexTime = System.currentTimeMillis() - indexStart
        log.info("[H3Agg] EMD - ES 인덱싱: {}건, {}ms", emdDocs.size, indexTime)
    ...
    fun reindexSgg(): Map<String, Any> {
        ...
        log.info("[H3Agg] SGG - ES에서 집계: {}건, {}ms", sggDocs.size, aggregateTime)

        val indexStart = System.currentTimeMillis()
        bulkIndexChunked(sggDocs, 10000)
        val indexTime = System.currentTimeMillis() - indexStart
        log.info("[H3Agg] SGG - ES 인덱싱: {}건, {}ms", sggDocs.size, indexTime)
    ...
    fun reindexSd(): Map<String, Any> {
        ...
        val sdDocs = aggregateFromEs("SGG", 5, "SD")
        val aggregateTime = System.currentTimeMillis() - startTime
        log.info("[H3Agg] SD - ES에서 집계: {}건, {}ms", sdDocs.size, aggregateTime)

        val indexStart = System.currentTimeMillis()
        bulkIndexChunked(sdDocs, 10000)
        val indexTime = System.currentTimeMillis() - indexStart
        log.info("[H3Agg] SD - ES 인덱싱: {}건, {}ms", sdDocs.size, indexTime)
    ...
    fun forcemerge(): Map<String, Any> {
        ...
        log.info("[H3Agg] forcemerge 완료: {}ms", elapsed)
