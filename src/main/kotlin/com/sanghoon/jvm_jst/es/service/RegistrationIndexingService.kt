...
import com.sanghoon.jvm_jst.rds.repository.RegistrationCursorRepository
...
class RegistrationIndexingService(
    ...
    fun reindex(): Map<String, Any> = runBlocking {
        ...
        val totalCount = registrationRepo.countAll()
        log.info("[Registration] 전체 건수: {}", totalCount)

        val boundaries = registrationRepo.findIdBoundaries(PARALLELISM)
        log.info("[Registration] {} 개 워커로 병렬 처리 시작, boundaries: {}", PARALLELISM, boundaries)
    ...
    private suspend fun processPartition(
        ...
        while (true) {
            ...
            val rows = registrationRepo.findByIdRange(minId, maxId, lastId, BATCH_SIZE)
            val t1 = System.currentTimeMillis()
            if (rows.isEmpty()) break

            log.debug("[Registration] W-{} DB 조회: {}ms ({}건)", workerIndex, t1 - t0, rows.size)

            val docs = rows.map { RegistrationDocument.fromRow(it) }
            ...
            if (globalProcessed % 50000 < rows.size) {
