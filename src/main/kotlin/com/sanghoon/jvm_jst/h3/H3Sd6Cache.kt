...
@Component
class H3Sd6Cache(
    private val repository: H3AggSd6Repository
) {
    private val log = LoggerFactory.getLogger(javaClass)

    // h3Index -> List<SdCellData>
    private val cache = ConcurrentHashMap<String, List<SdCellData>>()
    private val loadedKeys = ConcurrentHashMap.newKeySet<String>()

    companion object {
        private const val BYTES_PER_RECORD = 60L // sdCd(2자리) 포함 추정
    ...
    fun init() {
        Thread {
            log.info("H3Sd6Cache 프리로드 시작 (시도 {}개)", ALL_SIDO_CODES.size)
            ...
            log.info("H3Sd6Cache 프리로드 완료: H3셀={}개, 레코드={}개, {}MB, 총 {}ms",
    ...
    fun get(h3Indexes: Collection<String>): Map<String, List<SdCellData>> {
        val result = HashMap<String, List<SdCellData>>(h3Indexes.size)
    ...
    private fun loadFromDb(h3Indexes: List<String>, result: MutableMap<String, List<SdCellData>>) {
        val rows = repository.findByH3IndexesWithSd(h3Indexes)
