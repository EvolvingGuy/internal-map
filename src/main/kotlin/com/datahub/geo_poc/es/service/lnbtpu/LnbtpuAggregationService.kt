...
/**
 * LNBTPU (Land Nested Building Trade Point Uniform) Aggregation 서비스
 * 4개 파티션 인덱스 동시 쿼리 (lnbtpu_*)
 */
@Service
class LnbtpuAggregationService(
    ...
    companion object {
        const val INDEX_PATTERN = "lnbtpu_*"
    ...
    private fun aggregate(field: String, size: Int, bbox: BBoxRequest, filter: LcAggFilter): LcAggResponse {
        ...
        log.info("[LNBTPU Agg] field={}, regions={}, totalCount={}, hasFilter={}, index={}, ES took={}ms, total={}ms",
    ...
    private fun logProfile(response: org.opensearch.client.opensearch.core.SearchResponse<Void>, field: String) {
        ...
        log.info("[LNBTPU Profile] field={}, shards={}", field, shards.size)
        ...
        shards.forEach { shardProfile ->
            ...
            shardProfile.searches().forEach { search ->
                search.query().forEach { queryProfile ->
                    ...
                    log.info("[LNBTPU Profile] {} - {} - {}ms", shardId, queryProfile.type(), timeMs)
                }
            }
            shardProfile.aggregations().forEach { aggProfile ->
                val timeMs = String.format("%.2f", aggProfile.timeInNanos() / 1_000_000.0)
                log.info("[LNBTPU Profile] {} - agg:{} - {}ms", shardId, aggProfile.type(), timeMs)
            }
        }
    }
}
