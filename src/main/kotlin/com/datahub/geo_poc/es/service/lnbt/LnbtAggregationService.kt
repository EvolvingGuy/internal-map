...
/**
 * LNBT (Land Nested Building Trade) Aggregation 서비스
 * nested 타입 buildings, trades 배열에 대한 필터링 지원
 * 필터 없으면 LSRC(고정형 클러스터) 사용
 */
@Service
class LnbtAggregationService(
    ...
    private fun aggregate(field: String, size: Int, bbox: BBoxRequest, filter: LcAggFilter): LcAggResponse {
        ...
        log.info("[LNBT Agg] field={}, regions={}, totalCount={}, hasFilter={}, elapsed={}ms",
    ...
    private fun applyFilters(bool: BoolQuery.Builder, filter: LcAggFilter) {
        ...
        // Trade filters - nested query로 감싸서 적용
    ...
    /**
     * Trade 필터들을 nested query용 Query 리스트로 변환
     */
        ...
        shards.forEachIndexed { idx, shardProfile ->
            shardProfile.searches().forEach { search ->
                search.query().forEach { queryProfile ->
                    ...
                    log.info("[LNBT Agg Profile] field={}, 샤드[{}] {} - {}ms", field, idx, queryProfile.type(), timeMs)
                }
            }
        }
    }
