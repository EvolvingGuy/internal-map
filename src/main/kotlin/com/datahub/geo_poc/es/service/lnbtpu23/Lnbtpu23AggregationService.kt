...
/**
 * LNBTPU23 (Land Nested Building Trade Point Uniform 2-Index 3-Shard) Aggregation 서비스
 * 2개 파티션 인덱스 동시 쿼리 (lnbtpu23_*)
 */
@Service
class Lnbtpu23AggregationService(
    ...
    companion object {
        const val INDEX_PATTERN = "lnbtpu23_*"
    ...
    private fun aggregate(field: String, size: Int, bbox: BBoxRequest, filter: LcAggFilter): LcAggResponse {
        ...
        log.info("[LNBTPU23 Agg] field={}, regions={}, totalCount={}, hasFilter={}, index={}, ES took={}ms, total={}ms",
    ...
    private fun logProfile(response: org.opensearch.client.opensearch.core.SearchResponse<Void>, field: String) {
        ...
        log.info("[LNBTPU23 Profile] field={}, shards={}", field, shards.size)
        ...
        shards.forEach { shardProfile ->
            ...
            shardProfile.searches().forEach { search ->
                search.query().forEach { queryProfile ->
                    ...
                    log.info("[LNBTPU23 Profile] {} - {} - {}ms", shardId, queryProfile.type(), timeMs)
                }
            }
            shardProfile.aggregations().forEach { aggProfile ->
                val timeMs = String.format("%.2f", aggProfile.timeInNanos() / 1_000_000.0)
                log.info("[LNBTPU23 Profile] {} - agg:{} - {}ms", shardId, aggProfile.type(), timeMs)
            }
        }
    }
}
