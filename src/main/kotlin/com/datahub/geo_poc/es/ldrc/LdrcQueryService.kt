...
import com.datahub.geo_poc.model.BBox
import com.datahub.geo_poc.model.LdrcCluster
import com.datahub.geo_poc.model.LdrcResponse
import com.datahub.geo_poc.util.H3Util
...
class LdrcQueryService(
    ...
    companion object {
        ...
        val SIDO_NAMES = mapOf(
            11L to "서울특별시", 26L to "부산광역시", 27L to "대구광역시",
            28L to "인천광역시", 29L to "광주광역시", 30L to "대전광역시",
            31L to "울산광역시", 36L to "세종특별자치시", 41L to "경기도",
            43L to "충청북도", 44L to "충청남도", 46L to "전라남도",
            47L to "경상북도", 48L to "경상남도", 50L to "제주특별자치도",
            51L to "강원특별자치도", 52L to "전북특별자치도"
        )

        val LEVEL_RESOLUTION = mapOf("SD" to 5, "SGG" to 7, "EMD" to 10)
    }

    fun queryByBBox(swLng: Double, swLat: Double, neLng: Double, neLat: Double, level: String): LdrcResponse {
        ...
        if (h3Indexes.isEmpty()) {
            ...
            return LdrcResponse(level = level, h3Count = 0, clusters = emptyList(), totalCount = 0, elapsedMs = System.currentTimeMillis() - startTime)
        }
