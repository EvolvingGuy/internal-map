# LNBTPUNF (Land Nested Building Trade Point Uniform No Forcemerge)
필지 단위 파티션 인덱스 - LNBTPU와 동일 구조, forcemerge 미적용 (성능 비교용)

## 인덱스 정보
| Key          | Value                                                    |
|--------------|----------------------------------------------------------|
| IndexPrefix  | `lnbtpunf`                                               |
| Indices      | `lnbtpunf_1`, `lnbtpunf_2`, `lnbtpunf_3`, `lnbtpunf_4` |
| QueryPattern | `lnbtpunf_*`                                             | 와일드카드
| Shards       | 4                                                        | 파티션당
| Replicas     | 0                                                        |
| Distribution | PNU hash % 4                                             | 균등분배
| Forcemerge   | **미적용**                                               | LNBTPU와의 유일한 차이점

## LNBTPU와의 비교
| Feature      | LNBTPU              | LNBTPUNF            |
|--------------|---------------------|----------------------|
| IndexPattern | `lnbtpu_*`          | `lnbtpunf_*`        |
| Partition    | PNU hash % 4        | PNU hash % 4        | 동일
| Distribution | 균등분배             | 균등분배             | 동일
| Document     | nested B+T          | nested B+T           | 동일
| Forcemerge   | 인덱싱 후 자동 실행  | **미적용**           | 핵심 차이
| Segments     | 1 (병합됨)          | 다수 (자연상태)       | 세그먼트 수 차이
...
// [LNBTPUNF] P1:248 P2:252 P3:245 P4:255
```
...
| Method | Endpoint                      |
|--------|-------------------------------|
| PUT    | `/api/es/lnbtpunf/reindex`   | 전체 재인덱싱 (비동기, forcemerge 없음)
| GET    | `/api/es/lnbtpunf/count`     | 인덱스 문서 수 (파티션별)
| DELETE | `/api/es/lnbtpunf`           | 인덱스 삭제

### 집계 API
| Method | Endpoint                    |
|--------|-----------------------------|
| GET    | `/api/es/lnbtpunf/agg/sd`  | 시도별 집계
| GET    | `/api/es/lnbtpunf/agg/sgg` | 시군구별 집계
| GET    | `/api/es/lnbtpunf/agg/emd` | 읍면동별 집계

### 웹 페이지
| Method | Endpoint                     |
|--------|------------------------------|
| GET    | `/page/es/lnbtpunf/agg/sd`  | 시도별 집계 지도
| GET    | `/page/es/lnbtpunf/agg/sgg` | 시군구별 집계 지도
| GET    | `/page/es/lnbtpunf/agg/emd` | 읍면동별 집계 지도

## 인덱싱 방식

### 비동기 처리
```kotlin
// reindex만 비동기 (forcemerge 없음)
...
[1] ensureAllIndicesExist()
    └─ 4개 인덱스 생성 (lnbtpunf_1 ~ lnbtpunf_4)
           ↓
[2] EMD 분배
...
### 로그 형식
```
[LNBTPUNF] Worker-0 벌크 #1/87,748: 1,000/87,747,632 (0.0%) EMD=11110101 (1/377) → P1:248 P2:252 P3:245 P4:255, 건물 1,234건, 실거래 567건, 스텝 1,234ms, 누적 2.34s
```
...
val response = esClient.search({ s ->
    s.index("lnbtpunf_*")  // 와일드카드로 4개 파티션 동시 조회
        .size(0)
        .profile(true)      // 필수!
        .query { ... }
...
| Type            | Path                                                  |
|-----------------|-------------------------------------------------------|
| Document        | `es/document/land/LnbtpunfDocument.kt`                |
| Indexing        | `es/service/lnbtpunf/LnbtpunfIndexingService.kt`     |
| Aggregation     | `es/service/lnbtpunf/LnbtpunfAggregationService.kt`  |
| REST Controller | `controller/rest/LnbtpunfRestController.kt`           |
| REST Agg        | `controller/rest/LnbtpunfAggRestController.kt`        |
| Web Controller  | `controller/web/LnbtpunfAggWebController.kt`          |
| Template        | `templates/es/lnbtpunf/agg.ftl`                       |
| Docker          | `docker-compose.lnbtpunf.yml`                         | volume: `geo_poc_os_lnbtpunf`
