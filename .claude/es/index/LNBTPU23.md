# LNBTPU23 (Land Nested Building Trade Point Uniform 2-Index 3-Shard)
필지 단위 파티션 인덱스 - LNBTPU의 2분할 균등분배 버전, PNU hash % 2로 문서 균등 분산, 파티션당 3샤드

## 인덱스 정보
| Key          | Value                                          |
|--------------|------------------------------------------------|
| IndexPrefix  | `lnbtpu23`                                     |
| Indices      | `lnbtpu23_1`, `lnbtpu23_2`                     |
| QueryPattern | `lnbtpu23_*`                                   | 와일드카드
| Shards       | 3                                              | 파티션당
| Replicas     | 0                                              |
| Distribution | PNU hash % 2                                   | 균등분배

## LNBTPU와의 비교
| Feature      | LNBTPU             | LNBTPU23            |
|--------------|--------------------|---------------------|
| IndexPattern | `lnbtpu_*`         | `lnbtpu23_*`        |
| Partitions   | 4                  | 2                   |
| Shards/Part  | 4                  | 3                   |
| Total Shards | 16                 | 6                   |
| Distribution | PNU hash % 4      | PNU hash % 2       |
| Indices      | lnbtpu_1~4         | lnbtpu23_1~2        |

## 균등분배 알고리즘
```kotlin
// PNU 코드 기반 파티션 결정 (hash % 2 + 1)
fun getPartition(pnu: String): Int = (pnu.hashCode().absoluteValue % PARTITION_COUNT) + 1

// 인덱싱 시 각 배치별 파티션 분포 로그
// [LNBTPU23] P1:500 P2:500
```
...
| Method | Endpoint                      |
|--------|-------------------------------|
| PUT    | `/api/es/lnbtpu23/reindex`    | 전체 재인덱싱 (비동기)
| PUT    | `/api/es/lnbtpu23/forcemerge` | 세그먼트 병합 (비동기)
| GET    | `/api/es/lnbtpu23/count`      | 인덱스 문서 수 (파티션별)
| DELETE | `/api/es/lnbtpu23`            | 인덱스 삭제

### 집계 API
| Method | Endpoint                    |
|--------|-----------------------------|
| GET    | `/api/es/lnbtpu23/agg/sd`   | 시도별 집계
| GET    | `/api/es/lnbtpu23/agg/sgg`  | 시군구별 집계
| GET    | `/api/es/lnbtpu23/agg/emd`  | 읍면동별 집계

### 웹 페이지
| Method | Endpoint                     |
|--------|------------------------------|
| GET    | `/page/es/lnbtpu23/agg/sd`   | 시도별 집계 지도
| GET    | `/page/es/lnbtpu23/agg/sgg`  | 시군구별 집계 지도
| GET    | `/page/es/lnbtpu23/agg/emd`  | 읍면동별 집계 지도
...
[1] ensureAllIndicesExist()
    └─ 2개 인덱스 생성 (lnbtpu23_1 ~ lnbtpu23_2)
           ↓
[2] EMD 분배
...
### 로그 형식
```
[LNBTPU23] Worker-0 벌크 #1/87,748: 1,000/87,747,632 (0.0%) EMD=11110101 (1/377) → P1:500 P2:500, 건물 1,234건, 실거래 567건, 스텝 1,234ms, 누적 2.34s
```
...
val response = esClient.search({ s ->
    s.index("lnbtpu23_*")  // 와일드카드로 2개 파티션 동시 조회
        .size(0)
        .profile(true)   // 필수!
        .query { ... }
...
### 필터 있는 경우
2개 파티션 동시 쿼리 후 집계 → 균등 분배로 인한 병렬 처리 효율 극대화
...
| Type            | Path                                                |
|-----------------|-----------------------------------------------------|
| Document        | `es/document/land/Lnbtpu23Document.kt`              |
| Indexing        | `es/service/lnbtpu23/Lnbtpu23IndexingService.kt`    |
| Aggregation     | `es/service/lnbtpu23/Lnbtpu23AggregationService.kt` |
| REST Controller | `controller/rest/Lnbtpu23RestController.kt`         |
| REST Agg        | `controller/rest/Lnbtpu23AggRestController.kt`      |
| Web Controller  | `controller/web/Lnbtpu23AggWebController.kt`        |
| Template        | `templates/es/lnbtpu23/agg.ftl`                     |
| Docker          | `docker-compose.lnbtpu23.yml`                       | volume: `geo_poc_os_lnbtpu23`
